...
if __name__ == "__main__":
    ...
    # Train RL Agent for 50 Episodes
    num_episodes = 50
    episode_results = []
    ...
    for episode in range(num_episodes):
        ...
        # Evaluate Episode Metrics
        final_surplus = env.surplus
        ruin_probability = 1 if final_surplus < -100 else 0
        episode_results.append(
            {
                "Episode": episode + 1,
                "Final Surplus": final_surplus,
    ...
    results_df.to_csv("episode_results.csv", index=False)

    # Plot Results
    plt.figure(figsize=(12, 8))
    plt.plot(results_df["Episode"], results_df["Final Surplus"], label="Final Surplus", linewidth=2)
    plt.axhline(y=-100, color="red", linestyle="--", label="Ruin Threshold")
    plt.xlabel("Episodes", fontsize=18)
    plt.ylabel("Final Surplus", fontsize=18)
    plt.title("Final Surplus Across Episodes", fontsize=22)
    plt.legend(fontsize=16)
    plt.grid(True)
    plt.tight_layout()
    plt.savefig("final_surplus.png", dpi=300)
    plt.show()

    plt.figure(figsize=(12, 8))
    plt.plot(results_df["Episode"], results_df["Ruin Probability"], label="Ruin Probability", linewidth=2)
    plt.xlabel("Episodes", fontsize=18)
    plt.ylabel("Ruin Probability", fontsize=18)
    plt.title("Ruin Probability Across Episodes", fontsize=22)
    plt.legend(fontsize=16)
    plt.grid(True)
    plt.tight_layout()
    plt.savefig("ruin_probability.png", dpi=300)
    plt.show()
